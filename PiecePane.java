    ...
    private void checkBounds(Rectangle[] r) {
        ...
        for (Rectangle[] rect : rArray) {
            if (!(rect.equals(r)) && !(rect.equals(rArray.get(rArray.size() - 1)))) {
                ...
                for (int i = 0; i < 4; i++) {
                    // if
                    // (r.getBoundsInParent().intersects(rect.getBoundsInParent())&&r.getY()==rect.getY()-50&&r.getX()==rect.getX())
                    // {
                    // collisionDetected = true;
                    // }
                    if (r[0].getBoundsInParent().intersects(rect[i].getBoundsInParent())
                            && r[0].getY() == rect[i].getY() - 50 && r[0].getX() == rect[i].getX()) {
                        collisionDetected = true;
                    }
                    if (r[1].getBoundsInParent().intersects(rect[i].getBoundsInParent())
                            && r[1].getY() == rect[i].getY() - 50 && r[1].getX() == rect[i].getX()) {
                        collisionDetected = true;
                    }
                    if (r[2].getBoundsInParent().intersects(rect[i].getBoundsInParent())
                            && r[2].getY() == rect[i].getY() - 50 && r[2].getX() == rect[i].getX()) {
                        collisionDetected = true;
                    }
                    if (r[3].getBoundsInParent().intersects(rect[i].getBoundsInParent())
                            && r[3].getY() == rect[i].getY() - 50 && r[3].getX() == rect[i].getX()) {
                        collisionDetected = true;
                    }
                }
            }
        ...
        for (int i = 0; i < rArray.size() - 1; i++) {
            for (int j = 0; j < 4; j++) {
                for (int l = 0; l < 4; l++) {
                    if (r[j].getY() <= rArray.get(i)[l].getY() && r[j].getY() >= rArray.get(i)[l].getY() - 10
                            && r[j].getX() <= rArray.get(i)[l].getX() + 25
                            && r[j].getX() >= rArray.get(i)[l].getX() - 25) {
                        collisionDetected = true;
                    }
                }
    ...
    protected void move() {
        ...
        for (Rectangle[] r : rArray) {
            if (!(r.equals(rArray.get(rArray.size() - 1)))) {
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 4; j++) {
                        if (r[j].getX() >= rArray.get(rArray.size() - 1)[i].getX() - 49
                                && r[j].getX() <= rArray.get(rArray.size() - 1)[i].getX() + 49) {
                            if (r[j].getY() >= rArray.get(rArray.size() - 1)[i].getY() - 49
                                    && r[j].getY() <= rArray.get(rArray.size() - 1)[i].getY() + 49) {
                                changePos = true;
                            }
                        }
                    }
